      ...
      <div className="space-y-10">
        ...
        <Bloque titulo={ca ? 'Galetes' : 'Cookies'}>
          ...
          {real(ESTUDIO.analitica.ga4) ? (
            <p className="t-cuerpo !max-w-none">
              {ca
                ? "Aquest web fa servir Google Analytics per saber quanta gent el visita i quins cursos es miren més. L'etiqueta de Google es carrega a totes les pàgines, però amb el permís DENEGAT fins que tu ho acceptes al cartell que apareix en entrar: mentre està denegat, Google no escriu ni llegeix cap galeta i no t'identifica, només compta de manera anònima que algú hi ha passat. Si dius que no, es queda així. Si ho acceptes, Google desa dues galetes al teu navegador —«_ga» i «_ga_» seguit de l'identificador del web—, que caduquen als dos anys i serveixen per distingir visites. La teva resposta es guarda al teu propi navegador, i pots canviar-la quan vulguis amb el botó «Preferències de galetes» del peu de pàgina, que torna a obrir el cartell. No hi ha galetes de publicitat ni de xarxes socials. Hi ha, a més, una galeta tècnica que manté oberta la sessió del tauler de gestió: només la rep qui administra el web."
                : 'Esta web usa Google Analytics para saber cuánta gente la visita y qué cursos se miran más. La etiqueta de Google se carga en todas las páginas, pero con el permiso NEGADO hasta que tú lo aceptas en el aviso que aparece al entrar: mientras está negado, Google no escribe ni lee ninguna cookie y no te identifica, solo cuenta de forma anónima que alguien ha pasado. Si dices que no, se queda así. Si lo aceptas, Google guarda dos cookies en tu navegador —«_ga» y «_ga_» seguida del identificador de la web—, que caducan a los dos años y sirven para distinguir visitas. Tu respuesta se guarda en tu propio navegador, y puedes cambiarla cuando quieras con el botón «Preferencias de cookies» del pie de página, que vuelve a abrir el aviso. No hay cookies de publicidad ni de redes sociales. Hay, además, una cookie técnica que mantiene abierta la sesión del panel de gestión: solo la recibe quien administra la web.'}
            </p>
          ) : (
            <p className="t-cuerpo !max-w-none">
              {ca
